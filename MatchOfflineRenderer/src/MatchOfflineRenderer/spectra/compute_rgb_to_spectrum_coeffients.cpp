...
namespace MatchOfflineRenderer::spectra {
    // 预计算 RGB转换表, 计算时均使用double, 因为是预计算, 所以追求高精度

    std::unique_ptr<RGBToSpectrumTable> rgb_to_spectrum_table_sRGB { nullptr };
    std::unique_ptr<RGBToSpectrumTable> rgb_to_spectrum_table_DCI_P3 { nullptr };
    std::unique_ptr<RGBToSpectrumTable> rgb_to_spectrum_table_Rec2020 { nullptr };
    std::unique_ptr<RGBToSpectrumTable> rgb_to_spectrum_table_ACES2065_1 { nullptr };

    static constexpr int cie_sample_count_highp = 95;
    static constexpr int resolution = rgb_to_spectrum_table_resolution;
    ...
    #define N(x) (x / 106.8)
    static constexpr double cie_e[cie_sample_count_highp] = {
    ...
    static constexpr double cie_d50[cie_sample_count_highp] = {
    ...
    double cie_interp(const double *data, double x) {
        x -= lambda_min;
        x *= (cie_sample_count_highp - 1) / (lambda_max - lambda_min);
        int offset = (int)x;
        if (offset < 0)
            offset = 0;
    ...
    */
    ...
    void eval_residual(const double *coeffs, const double *rgb, double *residual, TableData *data) {
        ...
        for (int i = 0; i < cie_fine_sample_count; ++i) {
            ...
            double lambda =
                (data->lambda_table[i] - lambda_min) / (lambda_max - lambda_min);
    ...
    void gauss_newton(const double rgb[3], double coeffs[3], TableData *data, int it) {
        ...
        for (size_t i = 0; i < it; i++) {
            ...
            if (rv != 1) {
                MCH_ERROR("RGB {} {} {}", rgb[0], rgb[1], rgb[2])
                MCH_ERROR("-> {} {} {}", coeffs[0], coeffs[1], coeffs[2])
    ...
    static TableData *compute_colorapce(RGBColorSpaceType colorspace, int it) {
        ...
        double h = (lambda_max - lambda_min) / (cie_fine_sample_count - 1);
        ...
        for (size_t l = 0; l < 3; l ++) {
            ParallelFor(0, resolution, [&](size_t j) {
                MCH_INFO("ColorSpace: {} L: {} J: {}", to_string(colorspace), l, j)
                ...
                for (size_t y = 0; y < resolution; y ++) {
                    for (size_t x = 0; x < resolution; x ++) {
                        for (size_t c = 0; c < 3; c ++) {
                            data->coefficients[maxc][z][y][x][c] = math::Real { *ptr };
                            ptr ++;
                        }
                    }
                }
        ...
        // 输出到文件
        std::stringstream ss;
        // ss << "MatchOfflineRenderer/src/MatchOfflineRenderer/spectra/auto_generate/";
        ss << "auto_generate_code_" << to_string(colorspace) << "_table_data.cpp";
        auto filename = ss.str();
        ss.clear();
        ss.str("");
        ss << "rgb_to_spectrum_table_" << to_string(colorspace) << "_z_nodes";
        auto z_nodes_name = ss.str();
        ss.clear();
        ss.str("");
        ss << "rgb_to_spectrum_table_" << to_string(colorspace) << "_coefficients";
        auto coefficients_name = ss.str();

        FILE *f = fopen(filename.c_str(), "w");
        if (f == nullptr) {
            throw std::runtime_error("Could not create file!");
        }
        fprintf(f, "#include <MatchOfflineRenderer/spectra/rgb_convert_types.hpp>\n\n");
        fprintf(f, "namespace MatchOfflineRenderer::spectra {\n");
        fprintf(f, "    const ZNodeArray<math::Real> %s = std::array<math::Real, %d> {\n", z_nodes_name.c_str(), resolution);
        for (size_t i = 0; i < resolution; i ++) {
            fprintf(f, "        math::Real { %.9g },\n", data->z_nodes[i]);
        }
        fprintf(f, "    };\n\n");
        fprintf(f, "    const CoefficientArray<math::Real> %s = std::array {\n",
                coefficients_name.c_str());
        ptr = data->coefficients_buffer;
        for (size_t maxc = 0; maxc < 3; maxc ++) {
            fprintf(f, "        std::array {\n");
            for (size_t z = 0; z < resolution; z ++) {
                fprintf(f, "            std::array {\n");
    ...
    void GenerateRGBToSpectrumTables(const InitInfo &init_info) {
        ...
        MCH_WARN("Active {} Threads on {} Cores", int(nThreads * 1.5), nThreads)
        threadPool = std::make_unique<ThreadPool>(nThreads * 1.5);

        auto *sRGB_data = compute_colorapce(RGBColorSpaceType::esRGB, init_info.gauss_newton_iteration_step);
        rgb_to_spectrum_table_sRGB = std::make_unique<RGBToSpectrumTable>(sRGB_data->z_nodes, sRGB_data->coefficients);
        delete sRGB_data;

        auto *DCI_P3_data = compute_colorapce(RGBColorSpaceType::eDCI_P3, init_info.gauss_newton_iteration_step);
        rgb_to_spectrum_table_DCI_P3 = std::make_unique<RGBToSpectrumTable>(DCI_P3_data->z_nodes, DCI_P3_data->coefficients);
        delete DCI_P3_data;

        auto *Rec2020_data = compute_colorapce(RGBColorSpaceType::eRec2020, init_info.gauss_newton_iteration_step);
        rgb_to_spectrum_table_Rec2020 = std::make_unique<RGBToSpectrumTable>(Rec2020_data->z_nodes, Rec2020_data->coefficients);
        delete Rec2020_data;
        
        auto *ACES2065_1_data = compute_colorapce(RGBColorSpaceType::eACES2065_1, init_info.gauss_newton_iteration_step);
        rgb_to_spectrum_table_ACES2065_1 = std::make_unique<RGBToSpectrumTable>(ACES2065_1_data->z_nodes, ACES2065_1_data->coefficients);
        delete ACES2065_1_data;
    }

    void LoadRGBToSpectrumTables() {
        rgb_to_spectrum_table_sRGB = std::make_unique<RGBToSpectrumTable>(rgb_to_spectrum_table_sRGB_z_nodes, rgb_to_spectrum_table_sRGB_coefficients);
        rgb_to_spectrum_table_DCI_P3 = std::make_unique<RGBToSpectrumTable>(rgb_to_spectrum_table_DCI_P3_z_nodes, rgb_to_spectrum_table_DCI_P3_coefficients);
        rgb_to_spectrum_table_Rec2020 = std::make_unique<RGBToSpectrumTable>(rgb_to_spectrum_table_Rec2020_z_nodes, rgb_to_spectrum_table_Rec2020_coefficients);
        rgb_to_spectrum_table_ACES2065_1 = std::make_unique<RGBToSpectrumTable>(rgb_to_spectrum_table_ACES2065_1_z_nodes, rgb_to_spectrum_table_ACES2065_1_coefficients);
    }
}
